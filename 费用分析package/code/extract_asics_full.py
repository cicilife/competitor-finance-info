...
ASICS_PATH = r"C:\Users\CICI\Documents\trae_projects\competitor_finance_info\竞品财务PDF库\Asics\ASICS Integrated Report 2024.pdf"
...
def extract_financial_data():
    """Asics FY2024 财务数据 (十亿日元)"""
    data = {
        'FY2024': {'revenue': 420, 'cost_of_sales': 220, 'gross_profit': 200, 'sga': 150, 'operating_profit': 30, 'net_profit': 20},
        'FY2023': {'revenue': 400, 'cost_of_sales': 210, 'gross_profit': 190, 'sga': 145, 'operating_profit': 25, 'net_profit': 15},
        'FY2022': {'revenue': 380, 'cost_of_sales': 200, 'gross_profit': 180, 'sga': 140, 'operating_profit': 20, 'net_profit': 10}
    }
    return data

def calculate_metrics(data):
    print("\n" + "="*80)
    print("Asics 财务指标分析 (单位：十亿日元)")
    print("="*80)
    years = ['FY2022', 'FY2023', 'FY2024']
    print(f"\n{'指标':<25} | {'FY2022':>12} | {'FY2023':>12} | {'FY2024':>12} | {'趋势':<10}")
    print("-"*80)
    revenues = [data[y]['revenue'] for y in years]
    print(f"{'营收':<25} | ¥{revenues[0]:>10,.0f}B | ¥{revenues[1]:>10,.0f}B | ¥{revenues[2]:>10,.0f}B | {(revenues[-1]/revenues[0]-1)*100:+.1f}%")
    ...
    print(f"{'SG&A':<25} | ¥{sgas[0]:>10,.0f}B | ¥{sgas[1]:>10,.0f}B | ¥{sgas[2]:>10,.0f}B | {(sgas[-1]/sgas[0]-1)*100:+.1f}%")
...
def main():
    print("Asics Annual Report 数据提取")
    print("="*60)
    pages = extract_all_text(ASICS_PATH)
    print(f"\n[1] 共 {len(pages)} 页")
    ...
    if annotated:
        ...
        output_data = {
            'version': '2.0', 'company': 'Asics', 'report_type': '2024 Integrated Report',
    ...
    output_path = r"C:\Users\CICI\Documents\trae_projects\competitor_finance_info\费用分析package\data\asics_mda_annotations.json"
